treeJSON = d3.json('flareWithVeryLessNodes.json', function (error, treeData) {
  ...
  const rectW = 200;
  const rectH = 50;
  const nodeSize = [rectW + 10, rectH + 10];
  // size of the diagram
  ...
  function update(source) {
    ...
    nodeEnter
      ...
      .attr('width', (d) => d.width || rectW)
      .attr('height', (d) => d.height || rectH)
      .append('xhtml')
      .html(function (d) {
        return `<div class="root-node">${d.name}<br/>${d.label || ''}</div>`;
      });
    ...
    node
      .select('rect.nodeCircle')
      .attr('height', (d) => d.height || rectH)
      .attr('width', (d) => d.width || rectW)
      .style('fill', function (d) {
        return d.backgroundColor || 'purple';
      })
